...
### `sensitivityCLPM()` --- "what would my CLPM look like under trait variance?"

The CLPM's AR and CL estimates are biased when the data carry stable trait variance (between-person ICC > 0), because the lagged predictors then correlate with the residual --- the classic Hamaker, Kuiper & Grasman [-@hamaker_critique_2015] critique, formalized analytically in the CLPM chapter (univariate case: $b^{\text{CLPM}}_{xx} \approx \text{ICC}_x + (1 - \text{ICC}_x)\,b^{\text{true}}_{xx}$).
