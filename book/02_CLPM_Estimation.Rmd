...
Monte Carlo analysis varying the number of panel waves, and the following fixed paramters ($n = 2000; \beta_x = 0.4; \beta_y = 0.7; \omega_{x \rightarrow y} = 0.2; \omega_{y \rightarrow x} = 0.3$). 
...
fit = lavaan::lavaan(estimateRICLPM(waves = .y),
                     data = dat, 
...
ggplot(results_long, aes(x = estimate, y = waves_factor)) +
  ...
  # Add median lines for each distribution
  ...
  labs(
    ...
    subtitle = "Green dashed line = true value | Black line = median estimate",
...
results <- expand_grid(
  ...
  mutate(
    ...
    # This iterates over the data, x and waves y
    fit = map2(data, waves, ~{
      lavaan::lavaan(estimateLChange(waves = .y,
                                     variable_type = "bivariate",
  ...
  cols_label(
   ...
   tab_source_note(
    ...
    tab_footnote(
      footnote = "Bias = Mean(estimate) - True value, Relative Bias = (Bias / True value) * 100")
```
There are inherent benefits to change score specification, 

* Absent unit effects, interchangeable with CLPM. The minimum number of waves is two
* Effectively deals with unit effects $\alpha_i$ by differencing them out. The minimum number of waves is two.
* If there are unit effects, the minimum number of waves required is three to identify the model
