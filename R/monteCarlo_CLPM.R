...
#' This function combines the estimation and simulation of a CLPM over a fixed number of trials.
...
#' @param estimator Specify which estimator to use: "clpm" or "clpm_nolag"
...
monteCarloCLPM <- function(
  ...
  valid_estimators <- c("clpm", "clpm_nolag")
  ...
  if(dgp == "clpm") {
      ...
      for (j in 1:trials) {
        ...
        model_result <- tryCatch({
          ...
          withCallingHandlers({
            ...
            if (estimator == "clpm") {
              model_syntax <- estimateCLPM(waves = waves)
            } else if (estimator == "clpm_nolag") {
              model_syntax <- estimateCLPM_nolag(waves = waves)
            }
    ...
    # NEW: Generate from latent change model, fit CLPM
    simulation_parameters <- expand.grid(
      ar_x = ar_x,
      ar_y = ar_y,
      cl_x = cl_x,
      ...
      for (j in 1:trials) {
        ...
        model_result <- tryCatch({
          ...
          withCallingHandlers({
            ...
            # Choose model syntax based on estimator - FIT CLPM!
            if (estimator == "clpm") {
              model_syntax <- estimateCLPM(waves = waves)
            } else if (estimator == "clpm_nolag") {
              model_syntax <- estimateCLPM_nolag(waves = waves)
