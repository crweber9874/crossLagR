...
estimateRICLPM <- function(data = data,
                           time_varying_x = x_vars, # Follow ordering, x1 ...
                           time_varying_y = y_vars,
                           time_invariant_vars = NULL,
                           waves = 10) {
  if (length(time_varying_x) != waves || length(time_varying_y) != waves) {
    stop("The length of x_vars and y_vars must be equal to the number of waves.")
  }
