      ...
      for (j in 1:trials) {
        ...
        model_result <- tryCatch({
          ...
          withCallingHandlers({
            ...
            ar_x <- param_table[param_table$label == "ar_x", "est"]      # X autoregressive
            ar_y <- param_table[param_table$label == "ar_y", "est"]      # Y autoregressive
            cl_x_to_y <- param_table[param_table$label == "cl_xy", "est"] # X→Y cross-lagged
            cl_y_to_x <- param_table[param_table$label == "cl_yx", "est"] # Y→X cross-lagged
