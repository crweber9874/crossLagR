...
if(dgp == "riclpm") {
    simulation_parameters <- expand.grid(
      ...
      for (i in 1:nrow(simulation_parameters)) {
        for (j in 1:trials) {
          ...
          model_fit_y <- lm(within.y ~  xlagw + ylagw, dat)
          model_fit_x <- lm(within.x ~  xlagw + ylagw, dat)


          xvalue_x <-  model_fit_x$coefficients[["xlagw"]]
          yvalue_x <-  model_fit_x$coefficients[["ylagw"]]

          xvalue_y <-  model_fit_y$coefficients[["xlagw"]]
          yvalue_y <-  model_fit_y$coefficients[["ylagw"]]
    ...
    for (i in 1:nrow(simulation_parameters)) {
      for (j in 1:trials) {
        ...
        # Fit the OLS model and extract coefficients
        model_fit_y <- lm(within.y ~  xlag + ylag, dat)
        model_fit_x <- lm(within.x ~  xlag + ylag, dat)
