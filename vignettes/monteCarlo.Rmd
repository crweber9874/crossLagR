...
param_grid <- expand.grid(
  # The AR parameters represent level effects x -> change x
  ar_x = c(-0.15, -0.20),
  ar_y = c(-0.20, -0.25),
  # The level coupling parameters represent the cross lag x -> change y
  cl_x = c(0),
  cl_y = c( 0),
  
  # The Phi parameters represent the relationshp between change scores change x1 -> change x2
  phi_x = c(0, 0.15),
  phi_y = c(0, 0.15),
  
  # The change-parameters represent the coupling of proportional effects change x2 -> change y3
  change_x = c(0),
  change_y = c(0),
  
  # Sample size
  sample_size = c(1000, 5000)
)

# Fixed function - parameters match param_grid columns
run_monte_carlo_simulation <- function(ar_x, ar_y, cl_x, cl_y, 
                                       phi_x, phi_y, change_x, change_y,
                                       sample_size) {
  monteCarloCLPM(
    trials = 100,
    waves = 5,
    dgp = "lchange",         # Generate from latent change
    estimator = "clpm",      # Fit CLPM (misspecification study!)
    
    # Pass latent change parameters
    ar_x = ar_x,
    ar_y = ar_y,
    cl_x = cl_x,
    cl_y = cl_y,
    ...
    # Variance parameters for latent change dgp
    initial_var_x = 1,
    initial_var_y = 1,
