---
title: "Building a Within/Between Estimator"
author: "Chris Weber"
date: "2025-01-21"
...
Bearing this in mind, the Cross Lagged Panel Model (CLPM) can be written as a one-level (ADL) model with cross-lagged effects.
...
There are different variations to this general model, often to accomodate important characteristics in one's data. For instance, it is important to consider state versus trait variation in the data generating process.The **Random Intercept Cross Lagged** model follows the form, but includes a time invariant "trait" level parameter to account for subject characteristics that reverts back to a an equilibrium level. The autoregressive and cross lagged parameters correspond to time varying "state" effects, the expected change in $y_t$ or $x_t$ from each perturbation in $y_{t-1}$ or $x_{t-1}$, after accounting for the trait level effect, $\mu_i$ (Schuurman, Ferrer, Boer-Sonneschein, and Hamaker 2016)
...
-   \underbrace{\begin{bmatrix}
       \mu_{y,i}\\
       \mu_{x,i}\\
       \end{bmatrix}}*{Trait}\\ e*{y,it} \sim N(0, \sigma^2_y)\\ e\_{x,it} \sim N(0, \sigma^2_x)\\ \mu_{y,i} \sim N(\bar{\mu*x},* \sigma^2{\mu*y})\\ \mu{x,i} \sim N(\bar{\mu*x},* \sigma^2{\mu\_x})$$
...
The package for this project, $\texttt{crossLag}$ includes a function to simulate data from the Random Intercept Cross Lagged Panel Model. There's an additional function to pivot the data to long format, with lags for each variable, and within person effects.

```{r, echo = FALSE, message = FALSE, warning = FALSE}
library(dplyr)
library(brms)

df = simulate_riclpm(waves = 5,
                variance.p = 0.5,
                variance.q = 0.5,
                stability.p = 0.20,
                stability.q = 0.20,
